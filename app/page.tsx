...
export default function JobDashboard() {
  ...
  const [profile, setProfile] = useState({ name: '', role: '', location: '', preference: 'Hybrid', rawResume: '' });
    ...
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans">
      ...
      <aside className="w-full md:w-64 p-6 border-r border-gray-100 flex flex-col gap-8">
        ...
        <div className="flex bg-gray-50 p-1 rounded-lg">
          <button onClick={() => setActiveView('jobs')} className={`flex-1 text-sm py-2 rounded-md font-medium transition-colors ${activeView === 'jobs' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>
            Job Search
          </button>
          <button onClick={() => setActiveView('analytics')} className={`flex-1 text-sm py-2 rounded-md font-medium transition-colors ${activeView === 'analytics' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>
            My Analytics
          </button>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((n) => (
                <div key={n} className="p-6 border rounded-lg shadow-sm animate-pulse border-gray-100 bg-white">
                  ...
                  <div className="space-y-3 mb-6"><div className="h-4 bg-gray-100 rounded w-1/4"></div></div>
          ...
          {!isLoading && hasSearched && filteredJobs.length === 0 && (
            <div className="flex flex-col items-center justify-center p-12 mt-6 text-center border-2 border-dashed rounded-lg bg-gray-50 border-gray-200">
              ...
              <button onClick={() => { setSearchQuery(''); setLocationQuery(''); setWorkMode('Any Mode'); }} className="mt-6 px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-md">
                Clear all filters
              </button>
            </div>
          )}
            ...
            <div key={job.id || idx} className="p-6 border border-gray-100 rounded-xl hover:shadow-md transition-shadow bg-white flex flex-col md:flex-row justify-between gap-4">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
                  <span className="px-3 py-1 bg-gray-50 text-gray-600 text-xs rounded-full border border-gray-200">{job.source}</span>
                </div>
              </div>
              <div className="flex flex-col items-start md:items-end justify-between min-w-[140px]">
                <span className="px-2 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded border border-green-100 mb-4 md:mb-0">✓ Verified</span>
                <div className="flex gap-2 w-full md:w-auto mt-auto">
                  <button onClick={() => handleEmailHR('hr@example.com', job.title)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Email HR</button>
                  <a href={job.url || '#'} target="_blank" rel="noreferrer" className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Apply Now</a>
                </div>
              </div>
            </div>
          ))}
